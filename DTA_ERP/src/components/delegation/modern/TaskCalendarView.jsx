...
const TaskCalendarView = ({ tasks, onTaskClick }) => {
    ...
    const getStartOfWeek = (date) => {
        ...
        const day = d.getDay();
        const diff = d.getDate() - day + (day === 0 ? -6 : 1); // Adjust to Monday start like screenshot
        d.setDate(diff);
        d.setHours(0, 0, 0, 0);
        return d;
    };
                    ...
                    {Array.from({ length: 35 }).map((_, i) => {
                        ...
                        return (
                            <div key={i} className={`min-h-[100px] p-2 bg-white dark:bg-[#12161b] ${!isCurrentMonth ? 'opacity-30' : ''} hover:bg-slate-50/50 transition-all relative`}>
                                <span className={`text-xs font-black ${isToday(day) ? 'bg-emerald-500 text-white w-6 h-6 flex items-center justify-center rounded-full' : 'text-slate-400'}`}>
                                    {day.getDate()}
                                </span>
                                <div className="mt-1 space-y-1">
                                    {dayTasks.slice(0, 3).map((t, idx) => (
                                        <div 
                                            ...
                                            className="text-[9px] font-bold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-100 dark:border-emerald-500/20 truncate cursor-pointer hover:bg-emerald-100 transition-colors flex items-center gap-1"
                                        >
                                ...
                                <div key={i} className="p-1 min-h-[40px] flex flex-wrap gap-1">
                                    {getTasksForDay(day, null).map((task, idx) => (
                                        <div 
                                            ...
                                            className="bg-emerald-500/10 border border-emerald-500/20 rounded-md px-2 py-1 flex-1 min-w-[60px] max-w-full cursor-pointer hover:bg-emerald-500/20 transition-colors flex items-center justify-between gap-2"
                                        >
                                            <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 truncate">{task.taskTitle}</p>
                                            <div className="flex items-center gap-1 shrink-0">
                                                {task.voiceNoteUrl && <Mic size={8} className="text-emerald-500" />}
