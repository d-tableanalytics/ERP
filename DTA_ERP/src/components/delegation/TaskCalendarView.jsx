                ...
                <div className="flex items-center bg-slate-50 dark:bg-slate-900/50 p-1 rounded-xl border border-slate-200 dark:border-slate-800">
                    {['Day', 'Week', 'Month'].map(type => (
                        <button
                            ...
                            className={`px-6 py-2 rounded-lg text-sm font-black transition-all ${viewType === type ? 'bg-white dark:bg-slate-800 text-[#137fec] shadow-sm border border-slate-200 dark:border-slate-700' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            {type}
                        </button>
                    ))}
            ...
            {viewType === 'Month' ? (
                <div className="grid grid-cols-7 gap-px bg-slate-100 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-xl overflow-hidden">
                    ...
                    {Array.from({ length: 35 }).map((_, i) => {
                        ...
                        return (
                            <div key={i} className={`min-h-[100px] p-2 bg-white dark:bg-[#12161b] ${!isCurrentMonth ? 'opacity-30' : ''} hover:bg-slate-50/50 transition-all relative`}>
                                <span className={`text-xs font-black ${isToday(day) ? 'bg-[#137fec] text-white w-6 h-6 flex items-center justify-center rounded-full' : 'text-slate-400'}`}>
                                    {day.getDate()}
                                </span>
                                <div className="mt-1 space-y-1">
                                    {dayTasks.slice(0, 3).map((t, idx) => (
                                        <div 
                                            ...
                                            className="text-[9px] font-bold bg-[#137fec]/10 text-[#137fec] dark:text-[#137fec] px-1.5 py-0.5 rounded border border-[#137fec]/20 dark:border-[#137fec]/20 truncate cursor-pointer hover:bg-[#137fec]/20 transition-colors flex items-center gap-1"
                                        >
                                    ...
                                    {dayTasks.length > 3 && (
                                        <div className="text-[8px] font-black text-slate-400 pl-1">+{dayTasks.length - 3} more</div>
                                    )}
                                </div>
                            </div>
                        );
                                ...
                                <div key={i} className="p-1 min-h-[40px] flex flex-wrap gap-1">
                                    {getTasksForDay(day, null).map((task, idx) => (
                                        <div 
                                            ...
                                            className="bg-[#137fec]/10 border border-[#137fec]/20 rounded-md px-2 py-1 flex-1 min-w-[60px] max-w-full cursor-pointer hover:bg-[#137fec]/20 transition-colors flex items-center justify-between gap-2"
                                        >
                                            <p className="text-[10px] font-bold text-[#137fec] dark:text-[#137fec] truncate">{task.taskTitle}</p>
