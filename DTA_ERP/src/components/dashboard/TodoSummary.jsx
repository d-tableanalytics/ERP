  ...
  return (
    <div className="bg-bg-card border border-border-main rounded-2xl p-6 shadow-sm flex flex-col h-full">
      <div className="flex justify-between items-center mb-6">
        ...
        <button className="text-xs font-bold text-primary hover:underline px-2 py-1 rounded-lg bg-primary/5">
          View All
        </button>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {tasks.map((task, index) => (
          <div
            key={index}
            className="flex items-start gap-4 p-3 rounded-xl hover:bg-bg-main transition-colors group"
          >
            <div className="mt-0.5 cursor-pointer">
              <span
                className={`material-symbols-outlined text-[24px] ${task.completed ? "text-emerald-500" : "text-slate-300 group-hover:text-primary"} transition-colors`}
              >
            ...
            <div className="flex-1">
              <p
                className={`text-sm font-bold leading-tight ${task.completed ? "text-text-muted/50 line-through" : "text-text-main"}`}
              >
                {task.title}
              </p>
