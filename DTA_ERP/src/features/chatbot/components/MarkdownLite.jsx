      ...
      {lines.map((line, idx) => {
        ...
        return (
          <div key={idx} className={bullet || number ? 'flex gap-2 items-start' : ''}>
            {bullet && <span className="chatbot-markdown-marker mt-0.5 flex-shrink-0">•</span>}
            {number && <span className="chatbot-markdown-marker mt-0.5 flex-shrink-0">{trimmed.match(/^\d+/)[0]}.</span>}
