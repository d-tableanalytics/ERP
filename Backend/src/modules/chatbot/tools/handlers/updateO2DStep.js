...
const schema = {
  ...
  fromStep: { type: 'string', max: 255 },
  toStep: { type: 'string', max: 255 },
  remarks: { type: 'string', max: 1000 },
  confirmed: { type: 'boolean' },
  adminOverride: { type: 'boolean' },
};

module.exports = async function updateO2DStep(args, user, ctx = {}) {
  ...
  const result = await o2dService.updateStep(values, user);
  return {
    ...result,
    slot: result.order ? {
      lastEntity: 'o2d',
